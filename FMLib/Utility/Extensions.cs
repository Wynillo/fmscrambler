...
public static class Extensions
{
    ...
    {
        var offset1 = offset > -1 ? offset : 0;
        var count = length > -1 ? length : data.Length;
    ...
    {
        var bytesAmt = str.Length / 2;
        var byteArr = new byte[bytesAmt];
        var byteInd = 0;

        for (var i = 0; i < str.Length; i += 2)
        {
            byte.TryParse(str.Substring(i, 2), System.Globalization.NumberStyles.AllowHexSpecifier, null, out var res);
            byteArr[byteInd++] = res;
        }
    ...
    {
        for (var index = copyOffset; index < length; ++index)
        {
    ...
    public static Bitmap ProportionallyResizeBitmap(this Bitmap sourceBitmap, int maxWidth, int maxHeight)
    {
        ...
        var height = (int)(((double)maxWidth) / (((double)sourceBitmap.Width) / ((double)sourceBitmap.Height)));
        ...
        {
            var width = (int)(maxHeight * (((double)sourceBitmap.Width) / ((double)sourceBitmap.Height)));
            size = new Size(width, maxHeight);
        }

        var image = new Bitmap(size.Width + 1, size.Height + 1);
