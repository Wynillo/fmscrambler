...
class Track
{
    ...
    public void Write(Stream bf, string fileName)
    {
        ...
        try
        {
            ...
            if (Audio && WavFormat)
            {
                var header = MakeWavHeader(RealLength);
                stream.Write(header, 0, header.Length);
            }
            var sz = StartPosition;
            var sector = StartSector;
            long realsz = 0;

            var buf = new byte[BinChunk.SectorLength];
    ...
    {
        var fl = (float)realsz / (float)realLength;
        var msg = string.Format("{0:d4}/{1:d4} MB  [{2}] {3:d3}%",
    ...
    byte[] MakeWavHeader(long length)
    {
        ...
        var memoryStream = new MemoryStream(wavHeaderLen);
        using (var writer = new BinaryWriter(memoryStream))
        {
            // RIFF header
            writer.Write("RIFF".ToCharArray());
            var dwordValue = (uint)length + wavDataHlen + wavFormatHlen + 4;
    ...
    static long ToFrames(string time)
    {
        ...
        var mins = int.Parse(segs[0]);
        var secs = int.Parse(segs[1]);
        var frames = int.Parse(segs[2]);
